...
export class FnsController {
  ...
  @ApiOperation({ 
    ...
    description: 'Принимает QR код чека и токен авторизации. Из токена извлекается ID пользователя и ID сети (promotionId) для начисления кешбека в рамках конкретной промоакции. Система автоматически проверяет корректность домена и подсети.'
  })
  @ApiHeader({
    name: 'host',
    description: 'ID подсети (promotionId) из токена авторизации. Система автоматически проверяет корректность подсети и пользователя.',
    required: true,
  })
  @ApiResponse({ 
    status: 200, 
